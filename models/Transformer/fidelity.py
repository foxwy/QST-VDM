...
def POVMProbTable(gen):
    ...
    P = []
    samples_unique = [int2basestr(n, Na, l=Nq) for n in range(Na**Nq)]
    samples_unique = np.array(samples_unique)
    P = gen.p(samples_unique)
    P /= np.sum(P)
    
    return samples_unique, P
